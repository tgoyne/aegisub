...
void HunspellSpellChecker::ReadUserDictionary() {
	...
	try {
		agi::scoped_ptr<std::istream> stream(agi::io::Open(userDicPath));
		copy_if(
...
void HunspellSpellChecker::WriteUserDictionary() {
	...
	agi::fs::CreateDirectory(agi::Path::DirName(userDicPath));
...
std::vector<std::string> HunspellSpellChecker::GetLanguageList() {
	...
	agi::fs::FilesInDirectory(path, "*.dic").GetAll(dic);
	agi::fs::FilesInDirectory(path, "*.aff").GetAll(aff);
...
void HunspellSpellChecker::OnLanguageChanged() {
	...
	std::string custDicRoot = StandardPaths::DecodePath(OPT_GET("Path/Dictionary")->GetString());
	std::string dataDicRoot = StandardPaths::DecodePath("?data/dictionaries");
	...
	std::string affPath = str(boost::format("%s/%s.aff") % custDicRoot % language);
	std::string dicPath = str(boost::format("%s/%s.dic") % custDicRoot % language);
	userDicPath = str(boost::format("%s/user_%s.dic") % StandardPaths::DecodePath("?user/dictionaries") % language);
	if (!agi::fs::FileExists(affPath) || !agi::fs::FileExists(dicPath)) {
		affPath = str(boost::format("%s/%s.aff") % dataDicRoot % language);
		dicPath = str(boost::format("%s/%s.dic") % dataDicRoot % language);
	}
