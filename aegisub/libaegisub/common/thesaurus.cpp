...
static void split(String const& str, Char sep, Container *out) {
	...
	while ((pos = str.find(prev, sep)) != String.npos) {
		if (pos - prev > 1) {
			out->push_back(str.substr(prev, pos - 1));
		}
		prev = pos + 1;
	}
	if (pos + 1 < str.size()) {
		out->push_back(str.substr(pos + 1));
	}
}
...
: dat(io::Open(dat_path))
{
	...
	*idx >> entry_count;
	...
	offsets.rehash(entry_count);

	// Read the list of words and file offsets for those words
	for (line_iterator<std::string> iter(*idx, encoding_name), end; entry_count > 0 && iter != end; --entry_count, ++iter) {
...
void Thesaurus::Lookup(std::string const& word, std::vector<Entry> *out) {
	...
	std::tr1::unordered_map<std::string, int>::const_iterator it = offsets.find(word);
