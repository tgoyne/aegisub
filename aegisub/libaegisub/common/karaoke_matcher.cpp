...
karaoke_match_result auto_match_karaoke(std::vector<std::string> const& source_strings, std::string const& dest_string) {
	...
	while (!src.empty()) {
		...
		auto matches = romaji_to_kana(src);
	...
	for (size_t lookahead = 0; lookahead < dst_lookahead_max; ++lookahead) {
		...
		if (next != dst_end)
			translit = kana_to_romaji(dst->str() + next->str());
		if (!translit)
			translit = kana_to_romaji(dst->str());
		...
		for (auto const& syl : source_strings) {
			...
			if (!(starts_with(syl, dst->str()) || (translit && starts_with(syl, translit)))) continue;
