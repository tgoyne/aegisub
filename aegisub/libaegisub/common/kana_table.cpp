...
struct cmp_romaji_prefix {
	int cmp(agi::kana_pair const& kp, std::string const& romaji) const {
		return strncmp(kp.romaji, romaji.c_str(), strlen(kp.romaji));
	}
	bool operator()(agi::kana_pair const& kp, std::string const& romaji) const {
		return cmp(kp, romaji) < 0;
	}
...
const char *kana_to_romaji(std::string const& kana) {
	auto pair = boost::lower_bound(::kana_to_romaji, kana, cmp_kana);
	return pair == std::end(::kana_to_romaji) || strcmp(pair->kana, kana.c_str()) ? nullptr : pair->romaji;
}

boost::iterator_range<const kana_pair *> romaji_to_kana(std::string const& romaji) {
	auto pair = boost::equal_range(::romaji_to_kana, romaji.c_str(), cmp_romaji_prefix());
