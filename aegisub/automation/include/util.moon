{
  ...
  copy_deep: (tbl) ->
    ...
    copy = (val) ->
      ...
      seen[val] = tbl
      return {k, copy(v) for k, v in pairs val}
    return copy tbl
  ...
  -- Serialize a table containing only numbers and strings
  serialize: (tbl) ->
    sep = string.char(28)
    num_start = string.char(14)
    str_start = string.char(15)
    start = (v) -> if type(v) == 'number' then num_start else str_start

    table.concat ["#{k}#{sep}#{start(v)}#{v}" for k, v in pairs tbl], sep

  -- Deserialize a table serialized by the above
  deserialize: (str) -> {}
}
